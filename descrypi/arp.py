...
MACS_RE = re.compile(r"""
                    ^\S+[ ]
                    \((?P<ip>[^)]+)\)
                    [ ]at[ ]
                    (?P<mac>(?P<prefix>{prefix}):(?:{rest}))
                    """.format(prefix=MAC_PREFIX_RE, rest=MAC_OCTET_TRIPLE_RE),
...
def find_pi_mac_ips():
  ...
  return [(m.group('mac'), m.group('ip'), descrypi.ieee_ra.model_by_mac_prefix(m.group('prefix')))
