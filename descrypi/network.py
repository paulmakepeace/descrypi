  ...
  * Gateway/router address (gateway)
  ...
  def ifconfig(self, ifconfig):
    ...
    if match:
      ...
      self.network = str(ipaddress.IPv4Network("%s/%s" % (broadcast, netmask), strict=False))
    return self

  @classmethod
  def from_ifconfigs(cls, ifconfigs):
    """Return [Interface, ...] for UP ether interfaces."""
    return (cls(interface).ifconfig(block)
            for (interface, block)
            in cls.IFCONFIG_INTERFACES_RE.findall(ifconfigs)
...
def local_interfaces(interface=None):
  ...
  response = subprocess.Popen(
      command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT).stdout
